...
def extract_def(pts1, pts2, idx1, idx2):
    n = idx1.shape[0]
    tongjid = np.ndarray([n, 4])
    ...
    for i in range(n):
        ...
        tongjid[i, 0] = ax - bx
        tongjid[i, 1] = ay - by
        tongjid[i, 2] = np.arctan((ay - by) / (ax - bx + 104)) / 3.1415926 * 180
        tongjid[i, 3] = np.sqrt((by - ay) * (by - ay) + (bx - ax + 104) * (bx - ax + 104))
    return tongjid

def cmprn(tongjid, lie, min, max, bins ):
    tongjid[np.argsort(tongjid[:,lie]),:]
    h4 = np.histogram(tongjid[:, lie], bins = range(min, max, 2))
    h4 = h4[0]
    h4[0] = h4[0] -1
    h4[bins-2] = h4[bins -2] - 1
    ...
    cn1 = count+1
    cn2 = count+max_n
    tongjid = tongjid[cn1:cn2, :]

    return tongjid, max_n
...
if __name__ == '__main__':
    ...
    parser.add_argument('--input', type=str, default='./veintest/',
                        help='Image directory or movie file or "camera" (for webcam).')
    parser.add_argument('--weights_path', type=str, default='superpoint_v1.pth',
                        help='Path to pretrained weights file (default: superpoint_v1.pth).')
    parser.add_argument('--img_glob', type=str, default='*.bmp',  #################pgm
                        help='Glob match if directory of images is specified (default: \'*.bmp\').')
    parser.add_argument('--skip', type=int, default=1,
                        help='Images to skip if input is movie or directory (default: 1).')
    parser.add_argument('--show_extra', action='store_true',
                        help='Show extra debug outputs (default: False).')
    parser.add_argument('--H', type=int, default=168,
    ...
    fe = SuperPointFrontend(weights_path='superpoint_v1.pth',  # 权重的路径str
                            nms_dist=4,  # 非极大值抑制 int距离4
                            conf_thresh=0.015,  # 探测器阈值0.015
                            nn_thresh=0.7,  # 匹配器阈值0.7
                            cuda=True)  # GPU加速 默认false

    # Create a window to display the demo.
    dataset = np.ndarray([312, 6, 168, 104]).astype(np.float32)
    tong = np.ndarray([312, 15])
    file_path = 'C:/Users/Admin/PycharmProjects/pythonProject2/SuperPointPretrainedNetwork/veintest/'
   # file_path = 'D:/fingervein/database/polyu database 3126ROI/'
    for i in range(1, 313):
        for j in range(1, 7):
            img = cv2.imread(file_path + "%d_%d.bmp" % (i, j))
            img = img[:, :, 1]
            img = img.astype(np.float32) / 255
            # print(file_path + "%d_%d.bmp" % (i, j))
            dataset[i - 1, j - 1, :, :] = img

    fea_cell = np.empty((312, 6), dtype=object)
    pts_cell = np.empty((312, 6), dtype=object)



    for i in range(1, 313):
        for j in range(1, 7):
            pts1, desc1, heatmap1 = fe.run(dataset[i - 1, j - 1, :, :])
            fea_cell[i-1, j-1] = desc1
            pts_cell[i - 1, j - 1] = pts1
    ...
    for i in range(1, 313):
        ...
        for j in range(1, 6):
            for k in range(j + 1, 7):
                desc1 = fea_cell[i - 1, j - 1]
                desc2 = fea_cell[i - 1, k - 1]
                pts1 = pts_cell[i - 1, j - 1]
                pts2 = pts_cell[i - 1, k - 1]

                if desc1 is None or desc2 is None:
                    tong[i - 1, count] = 0
                else:
                    match = nn_match_two_way(desc1, desc2, 1/1.4)
                    tongjid = extract_def(pts1, pts2, match[0], match[1])
                    tongjid, max_n = cmprn(tongjid, 2, -45, 45, 45)
                    tongjid, max_n = cmprn(tongjid, 3, 0, 300, 150)
                    tongjid, max_n = cmprn(tongjid, 0, -100, 100, 100)
                    tongjid, max_n = cmprn(tongjid, 1, -150, 150, 150)

                    tong[i - 1, count] = max_n
                   # tong[i - 1, count] = match.shape[1]
                count = count + 1
        count = 0

    np.savetxt('tong.txt', tong)


    yi = np.ndarray([1866, 1866])
    ...
    for i in range(1, 312):
        ...
        for k in range(1, 7):
            count = 0
            for j in range(i+1, 313):
                for l in range(1, 7):
                    desc1 = fea_cell[i-1, k-1]
                    desc2 = fea_cell[j - 1, l - 1]
                    pts1 = pts_cell[i - 1, k - 1]
                    pts2 = pts_cell[j - 1, l - 1]
                    if desc1 is None or desc2 is None:
                        yi[count1, count] = 0
                    else:
                        match = nn_match_two_way(desc1, desc2, 1/1.4)
                        ...
                        tongjid, max_n = cmprn(tongjid, 0, -100, 100, 100)
                        tongjid, max_n = cmprn(tongjid, 1, -150, 150, 150)
                        yi[count1, count] = max_n
                       # yi[count1, count] = match.shape[1]
                    count = count + 1
            count1 = count1 + 1
